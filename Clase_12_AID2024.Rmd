...
En el caso de variables categóricas se presentaron los test de independencia y homogeneidad, así como también los análisis de correspondencia.
...
ggplot(new_df, aes(x=ks, y=acc)) + geom_line(aes(color = sub_data,linetype=sub_data)) +
        theme +labs(x='Cantidad de vecinos', y='Métrica de performance (accuracy)', 
...
Las comparaciones las voy a hacer con la métrica de AUC. Como knn no tiene threshold (sólo cuenta cuántos vecinos hay de cada etiqueta), no es posible calcular AUC.
